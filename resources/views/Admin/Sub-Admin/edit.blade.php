<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel1">{{ translate('edit_sub_admin') }}</h5>
            <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"
                onclick="return close_or_clear();"></button>
        </div>
        <div class="modal-body" id="myModal">
            <form class="form-bookmark" method="post" action="{{ route('sub-admin.update', $data->id) }}"
                id="subadmin-edit-form">
                @csrf
                <div class="row g-2">
                    ...
                    <div class="col-sm-6">
                        ...
                        <select name="gender" aria-label="{{ translate('select_a_gender') }}"
                            ...
                            <option value="male" @if ($data->gender == 'Male') selected = "selected" @endif>
                                {{ translate('male') }}</option>
                            <option value="female" @if ($data->gender == 'Female') selected = "selected" @endif>
                    ...
                    <div class="col-sm-4">
                        <label class="form-label" for="profile_image">{{ translate('profile_image') }}
                            (Accept:png,jpg,jpeg) <span class="text-danger">*</span> </label>
                        <input class="form-control" id="profile_image" type="file" name="profile_image"
                            accept=".png, .jpg, .jpeg">
                    </div>
                    <div class="col-sm-2">
                        <label class="form-label" for="profile_image"></label><br>
                        <img src="@if (!empty($data->profile_image)) {{ static_asset('profile_image/' . $data->profile_image) }}@else{{ static_asset('/admin/assets/images/user/user.png') }} @endif"
                    ...
                    <div class="col-sm-6">
                        <label class="form-label"> {{ translate('select_role') }} <span
                                class="text-danger">*</span></label>
                        <select name="select_role" aria-label="{{ translate('select_a_role') }}"
                            data-placeholder="{{ translate('select_a_role') }}" class="form-select">
                            @foreach ($roles as $role)
                                <option value="{{ $role->id }}"
                                    @if ($role->id == @$data['role']['role_id']) selected="selected" @endif>{{ $role->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-sm-6">
                        <label class="form-label"> {{ translate('country_id') }} <span
                                class="text-danger">*</span></label>
                        <select name="country_id" aria-label="{{ translate('country_id') }}" class="form-select">
